...
@interface NJKAssetsCollectionTableViewCell()

@property (nonatomic, strong) UILabel *detailLabel;

@end

@implementation NJKAssetsCollectionTableViewCell
...
//    [self configCellWithCoverImage:entity.coverImage title:entity.title imageCount:entity.imageCount];
//}
...
//    image = [UIImage imageWithCGImage:posterImage scale:0 orientation:UIImageOrientationUp];
//    
//    NSString *title = [assetsGroup valueForProperty:ALAssetsGroupPropertyName];
//    NSString *imageCount = [NSString stringWithFormat:@"%@", @([assetsGroup numberOfAssets])];
//    
//    [self configCellWithCoverImage:image title:title imageCount:imageCount];
//}
...
- (void)configCellWithCoverImage:(UIImage *)coverImage
    ...
    if (imageCount) {
        [detailAttributedString appendAttributedString:[[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@", @(imageCount)] attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:14] , NSForegroundColorAttributeName : [UIColor grayColor]}] shouldWrap:YES];
    }
